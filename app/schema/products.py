...
class DimensionsCM(BaseModel):
    ...
    width: Annotated[
        float,
        Field(gt=0, strict=True, description="width in cm")
    ]
    height: Annotated[
        float,
        Field(gt=0, strict=True, description="height in cm")
    ]
...
class Seller(BaseModel):
    ...
    name: Annotated[
        ...
        Field(
            ...
            max_length=60,
            title="Seller Name",
            description="Seller name (2-60 chars).",
...
class Product(BaseModel):
    ...
    # Checking the correct SKU format for SKU FIELD in shcema 
    # EX =  XIAO-359GB-001

    # Field Validator can be implemented on only 1 Field resp at time.
    ...
    # Model Validator Helps to validate complete model 
    # It helps to validate multiple fields at a time
    ...
    # computed_field used to compute/calculate any value based on the field
    # It creates new filed in schema after computation
